...
def test_user_function_lane_requires_live_fixture_urls_in_ci():
    repository_root = Path(__file__).parents[3]
    workflow = (
        repository_root / ".github/workflows/integration-fabric.yaml"
    ).read_text()
    ...
    assert function_tests.count("fabric_test_function_environment(") == 3
    assert (
        "FABRIC_FUNCTION_INTEGRATION_REQUIRED: "
        "${{ matrix.filter == 'integration-fabric-functions' }}"
    ) in workflow
...
def test_live_power_bi_suite_cancels_an_enhanced_refresh():
    ...
    assert "fabric_pbi_refresh_cancel(refresh)" in tests
