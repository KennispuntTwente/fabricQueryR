...
fields remain directly readable with `$`, while `$get()` and
`$as_list()` provide collision-safe and plain-record access. Record
...
is reused while the object remains in the current R process. An
explicitly supplied `token`, `tenant_id`, `client_id`, or `auth_args`
takes precedence.
...
methods. Other discovered types remain `FabricItem` objects with
`$details()` and record access, avoiding methods that cannot operate
