...
Arrow streams use the bearer token captured when the stream is opened.
Consume them promptly; if a long scan outlives the token, reopen the
exact snapshot with
`version = attr(stream, "fabric_delta_snapshot_version")` and a fresh
token.
