...
Timestamp partition values without an offset are interpreted as UTC,
matching the Fabric test runtime. Metadata must declare Parquet with no
provider-specific options. Unknown reader features, malformed recursive
schemas/commits, catalog-managed commits, and absolute paths outside
OneLake fail before any rows are returned. A caller-supplied `dest_dir`
