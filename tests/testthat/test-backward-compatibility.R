...
test_that("SQL legacy catalog fallback coexists with target inference", {
  expect_identical(
    fabric_sql_database_argument(
  ...
  fabric_sql_connect(
    paste0(
      "Server=server.datawarehouse.fabric.microsoft.com,1444;",
      "Database=Warehouse"
    ),
    token = "token",
    verbose = FALSE
  ...
  expect_identical(captured[[1L]]$database, "Lakehouse")
  expect_identical(captured[[1L]]$Port, 1433L)
  expect_identical(captured[[2L]]$database, "Warehouse")
  expect_identical(captured[[2L]]$Port, 1444L)
})
