...
A Fabric user data function is similar to calling a function in another
R package, except its Python code runs in Fabric. R sends named inputs
to a secure web address and receives a structured result.

fabricQueryR invokes functions that are already defined, published, and
enabled for public access. It does not create or deploy the function.
This guide starts with one small scalar call before introducing nested
inputs, retries, and security limits.
...
Parameter names must match the published Python function. For a function
with `customerName` and `priority` inputs:
...
`Succeeded` means the function completed. The output shape is defined by
the published function, so inspect `result$output` before building
downstream R code around it.
