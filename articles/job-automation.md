...
Non-UTC firing remains unverified in the package’s persistent Fabric
sandbox. Enabled Amsterdam schedules have not produced the expected run
within the test window, and the cause has not been established. Actual
monthly recurrence and nonexistent or repeated local times at
daylight-saving transitions also lack live execution evidence. Before
relying on one of these configurations, verify an actual scheduled run
using `$instances()` and `$status()`. Successful schedule creation only
confirms that Fabric accepted the configuration.
