...
fabricQueryR supports both on-demand and recurring item jobs through the
Microsoft Fabric Job Scheduler APIs. This guide uses a discovered
notebook so the workspace ID, item ID, type, and workspace-specific API
endpoint travel together.
...
The examples use the package’s normal authentication flow. Reading
history and schedules needs an item read permission. Running jobs needs
item execute permission. Creating or updating a schedule needs item
execute and read-write permissions; deletion needs item read-write
permission. A workspace Contributor role or direct item Write permission
is normally needed to change schedules.
