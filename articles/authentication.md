...
For a GraphQL service principal, also set the function’s audience:
...
- **OneLake and Delta.** Grant a workspace role, item permission, or a
  OneLake data-access role. Admin, Member, and Contributor roles can
  write broadly; granular OneLake Read or ReadWrite roles are safer for
  narrower access. See [OneLake security best
...
workspace, item, and workload permissions before changing the login
