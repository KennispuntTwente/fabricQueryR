...
item *Read* grant also exposes metadata without granting the underlying
OneLake data. For example, a semantic model normally needs Read and
Build permission. A direct Lakehouse read needs an Admin, Member, or
Contributor workspace role; item *Read* plus *ReadAll*; or, when OneLake
security is enabled, item *Read* plus membership in a OneLake role that
grants *Read* on the target data. Detailed workload guidance appears
...
| User data functions as a delegated user | `https://analysis.windows.net/powerbi/api/UserDataFunction.Execute.All` |
| User data functions as a service principal | `https://analysis.windows.net/powerbi/api/.default` |
...
1.  For service principals and managed identities, a Fabric
    ...
    APIs* under *Admin portal \> Tenant settings \> Developer settings*.
    Scope the setting to a dedicated Entra security group where
    possible. See [Fabric identity
