# Update data and refresh a semantic model

Semantic models serve report-ready data to Power BI and Fabric reports.
After an upstream load finishes, fabricQueryR can start the model
refresh, monitor it, preserve diagnostic details, and link to Fabric’s
refresh detail page.
...
Starting or cancelling a refresh requires `Dataset.ReadWrite.All` and
Write permission on the semantic model. Status and history use
`Dataset.Read.All` or `Dataset.ReadWrite.All`, although Microsoft also
requires model Write permission for refresh history. Service principals
are supported when the tenant setting and model/workspace access allow
them. Do not set an email notification option for a service-principal
request.
...
The returned handle stores its target and in-process sign-in context. It
can be passed to status, wait, or cancel without copying IDs.
Credentials are not persisted when a handle is serialized; authenticate
again after restoring one in another R process.
...
The refresh submission `timeout` is an `HH:MM:SS` limit for each
attempt. Each retry gets a new attempt limit, but Power BI limits the
whole operation to 24 hours. The wait function’s numeric `timeout`
remains a separate client limit.

Use `Transactional` to preserve the previous model unless the entire
refresh succeeds. `PartialBatch` commits each processing command
separately, so a failure can leave only part of the new data loaded or a
table empty. It also cannot be combined with
`apply_refresh_policy = TRUE`.

Power BI permits only one active refresh per semantic model. A second
request is rejected rather than queued behind the first.
...
`CompletedWithWarnings`, `Failed`, `TimedOut`, `Cancelled`, and
`Disabled`.
...
Each attempt retains Data or Query phase information, timestamps,
execution metrics, and a parsed service exception. The complete response
remains in `raw`, so new Microsoft fields are still inspectable.
