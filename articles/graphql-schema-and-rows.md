...
JSON whole numbers beyond R’s exact double range are decoded as
character values. The collector preserves those strings. If the same
field has smaller R integer values on another page, it promotes those
smaller values to character too; it never converts the large value to an
inexact double. Other incompatible scalar changes stop with an error so
a schema change cannot silently alter the analysis.

GraphQL can return data and errors together. The rows remain usable,
while the combined service errors stay in `attr(products, "errors")` and
their count is shown when the tibble prints.
...
marks completion only after the cursor callback reports that no next
page exists. Reaching `max_pages` raises a
`fabric_graphql_pagination_error` whose `pages` field is marked
incomplete. If those pages are passed to
[`fabric_graphql_collect()`](https://kennispunttwente.github.io/fabricQueryR/reference/fabric_graphql_collect.md),
collection raises a `fabric_graphql_collection_error` instead of
returning an apparently complete tibble:
...
Fabric currently defaults each page to 100 items and limits pagination
to 100,000 items. A response is limited to 64 MB, one request to 100
seconds, and query nesting to 10 levels. Use smaller pages and stable
filtered partitions for large results rather than treating a
service-limit response as completion.
