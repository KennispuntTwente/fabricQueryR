...
`fabric_user_data_functions()` discovers the containing item and returns a
read-only `FabricItem` R6 object with service fields and a `$details()` method;
`$details()` calls `fabric_item()`. Microsoft's item response does not include
enough information to derive each public function URL, so pass the copied URL
to `fabric_function_invoke()`.
