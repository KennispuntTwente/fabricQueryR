...
endpoint, and the **tenant admin setting for the Livy API enabled**. A delegated
caller needs all four of these Microsoft Entra scopes:
...
service principal to the workspace as a Contributor. `fabricQueryR` requests
the four delegated scopes automatically for its normal interactive sign-in;
client-credentials authentication uses the Fabric `.default` audience.
...
If Fabric returns HTTP 403, check the tenant setting, the workspace Contributor
role, and—when using delegated authentication—the four scopes above before
changing the endpoint or Spark code. See Microsoft's
