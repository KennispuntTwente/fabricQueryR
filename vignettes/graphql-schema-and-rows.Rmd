...
`fabric_graphql_paginate()` marks a result complete only after the API reports
that no next page exists. If a page limit is reached first, collection raises
an error rather than returning an apparently complete tibble. The partial rows
...
For large results, use smaller pages and stable filtered partitions. See the
