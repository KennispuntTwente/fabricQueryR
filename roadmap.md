...
- The integration fixture matrix for basic, partitioned, V1/V2 checkpointed,
  ...
  row-tracking, shallow-clone, nested/exact-numeric, Variant, and
  Warehouse-export tables passes, or unsupported features fail before data is
  returned.
...
Microsoft has announced that the beta notebook API used to retrieve the richer
workload-specific job status will be deprecated on **April 1, 2028**. The
submission route already uses the release contract with `beta=false`; only the
enriched status lookup still calls the beta route before falling back to Core
...
- By the fourth quarter of 2027, make the supported stable route primary and
  retain the beta lookup only as a compatibility fallback if it is still needed.
- Remove all `beta=true` status requests in a release before April 1, 2028, and
  keep the current failure/exit-value reconciliation covered by integration
