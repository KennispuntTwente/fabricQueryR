...
4.  Work with general OneLake files using `fabric_onelake_list()`,
    `fabric_onelake_metadata()`, `fabric_onelake_download()`,
    `fabric_onelake_upload()`, and the explicitly enabled
    `fabric_onelake_delete()`.

5.  Execute a Livy API query: `fabric_livy_query()`. With this, you can
...
7.  Execute a GraphQL document against an `API for GraphQL` item:
...
# The Power BI "Dataset Execute Queries REST API" tenant setting must be
# enabled. Each request returns one table and is limited to 100,000 rows or
# 1,000,000 values, 15 MB, and 120 requests per minute per user. Service
# principals cannot query semantic models with RLS or SSO enabled.
